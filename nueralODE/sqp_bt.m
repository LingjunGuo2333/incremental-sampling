...
function [info, variables] = sqp_bt(P, params)
    ...
    S_set = params.S_set;                % set the current sample set
    output_dir = params.dir;             % set output directory
    checkpoint      = 1;                 % used for show_y below
    show_y          = [];                % collect preditions of the model when ekkt_res is first less than 0.1, 0.01, 0.001, ...
    ekkt_res_list   = [];                % list for kkt residual for full sample problem
    g_eval_list     = [];                % list for accumulated gradient evaluations corresponds to the ekkt residual.
    ...
    len_S           = length(S_set);
    iter            = 0; 
    tau             = params.tau;      % penalty parameter for merit function
    % caluate objective & constraint for its value & Jacobian
    [fx,~]          = P.evaluateObjectiveFunction(x0) ;
    [gx,~]          = P.evaluateObjectiveGradient(x0, "true") ;          % size n by 1
    [cx,~]          = P.evaluateConstraintFunctionEqualities(x0, S_set);
    [Jx,~]          = P.evaluateConstraintJacobianEqualities(x0, S_set); % size 1 by n
    count_geval     = len_S;
    ...
    while iter < params.maxit
        ...
        write_file(file_name, 0, iter, search_dire, stepsize, ...
                        ...
                        merit-e_merit,rk, fx, norm(cx,'Inf'), norm(ecx,'Inf'), ...
                        norm(cx,'Inf')-norm(ecx,'Inf'),tau);
        % save the predition when KKT of full sampled problem is less than
        % .1, .01, .001, ...
