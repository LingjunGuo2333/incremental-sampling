...
The third part of Assumption~\ref{ass.subproblems} is an essential element for proving our desired worst-case sample complexity properties of Algorithm~\ref{alg.pcsm}.  Related to this, the following comment is important, so we emphasize it as a remark.
...
Our first lemma leverages Assumption~\ref{ass.bounded.distribute} in order to provide bounds that are similar to those in the assumption, except that they are with respect to $c_\Scal$ and its derivatives for given $\Scal \subseteq [N]$.  The resulting bounds depend on the sample size $|\Scal|$.
...
\blemma\label{lemma.bound.c.y.L}
...
\begin{proof} 
%First, \cite[Theorem 1.1]{nakatsukasa2010absolute} gives us a bound on the difference between the smallest singular values, i.e. $|\sigma_m(\nabla c_\Scal(x))-\sigma_m(\nabla c(x)) |\le \|\nabla c_\Scal(x)-\nabla c (x)\|_2$. Combining it with (\ref{lemma.saa.nabla_c}) we have
%\begin{equation}
...
\bequation\label{eq.nabla.c.acute.formula}
	\begin{aligned}
		...
		&=\nabla c (x)\nabla c (x)^\dag \nabla c_\Scal(x)\\
		&=\nabla c (x)\nabla c (x)^\dag\left(\nabla c (x)+ \nabla c_\Scal(x)-\nabla c (x)\right)\\
		&=\nabla c (x)\left(I_m+\underbrace{\nabla c (x)^\dag\left(\nabla c_\Scal(x)-\nabla c (x)\right)}_{(\ref{eq.nabla.c.acute.formula}.1)}\right).
	\end{aligned}
	\eequation
...
\label{ineq.define.iii}
	\|y(x)-y_\Scal(x)\|
	\le \underbrace{\(\frac{\|\nabla c (x)^\dag\|\|\nabla c (x)-\nabla c_\Scal(x)\|}{1-\|\nabla c (x)^\dag\|\|\nabla c (x)-\nabla c_\Scal(x)\|}\)}_{(\ref{ineq.define.iii}.1)}\|y(x)\|.
\eequation
...
\min\left\{\frac{1}{3\eta_1}, \frac{\alpha}{2\sqrt{\gamma_c+\kappa_{\nabla f}^2\eta_1^2}},
 \frac{ \beta}{2\sqrt{\eta_3^2+2\beta\(\beta\eta_1^2+2\eta_1\eta_3\)}}
 \right\},
\eequation
the problem (\ref{prob.opt.S}) is $(\alpha_\Scal,\beta_\Scal)$-morse, where
\bequationNN
\bcases
\alpha_\Scal=\alpha- \sqrt{\gamma_c+\kappa_{\nabla f}^2\eta_1^2}g_\Scal\ge\half\alpha>0\text{ and }\\
...
\begin{proof}
%By (\ref{lemma.saa.nabla_c}) and triangle inequality, we have that for any $x\in\R{n}$,
%\bequationNN
%\|\nabla c_\Scal(x)\|\le\|\nabla c(x)\|+\|\nabla c_\Scal(x)-\nabla c(x)\| \le \(1+\sqrt{ \gamma_{\nabla c}}g_\Scal \) \|\nabla c(x)\|.
%	\eequationNN
...
\label{ineq.L.grad,y.diff}
	\left\|\nabla_yL(x,y)-\nabla_yL_{\Scal}(x,y_{\Scal})\right\| =\| c(x) -  c_\Scal(x) \|\le\sqrt{\gamma_c} g_\Scal.
\eequation
...
\bequation\label{ineq.nablaL.LS}
 \begin{aligned}
 	...
 	&=\sqrt{\left\|\nabla_x L(x,y)-\nabla_x L_{\Scal}(x,y_{\Scal})\right\|^2+\left\|\nabla_y L(x,y)-\nabla_y L_{\Scal}(x,y_{\Scal})\right\|^2} \\
 	&\le  \sqrt{\gamma_c+\frac{\gamma_{\nabla c} \kappa_{\nabla c}^2\kappa_{\nabla f}^2}{\sigma_{\min}^2}}g_\Scal.
 \end{aligned}
 \eequation
...
&\left|d_\Scal^T\nabla^2_{xx}L_\Scal(x,y_\Scal)d_\Scal\right|\\
	...
	&\ge \underbrace{\left| d_\Scal^T\nabla^2_{xx}L (x,y )d_\Scal\right|}_{(\ref{ineq.dSLdS}.1)}-\underbrace{\left|d_\Scal^T\nabla^2_{xx}L_\Scal(x,y_\Scal)d_\Scal-d_\Scal^T\nabla^2_{xx}L (x,y )d_\Scal\right|}_{(\ref{ineq.dSLdS}.2)}.
\end{aligned}
\eequation
...
\min\left\{
\frac{1}{3\eta_1},\frac{\alpha}{4\sqrt{\gamma_{c}+\kappa_{\nabla f}^2\eta_1^2}},
\frac{\beta}{4\sqrt{\eta_3^2+\beta\(\beta\eta_1^2+2\eta_1\eta_3\)}}
\right\},
\eequation
...
\begin{aligned}
	\|\nabla_xL_{\Scal_{k+1}}(x_{\Scal_{k}},z_{\Scal_{k+1}})-\nabla_xL_{\Scal_{k}}(x_{\Scal_{k}},y_{\Scal_{k}})\|\le 2\kappa_{\nabla f}\eta_1g_{\Scal_k}.
\end{aligned}
\eequation
...
&\le \underbrace{\left|d_{\Scal_{k+1}}^T\nabla^2_{xx}L_{\Scal_{k+1}}(x_{\Scal_{k}},z_{\Scal_{k+1}})d_{\Scal_{k+1}}-d_{\Scal_{k+1}}^T\nabla^2_{xx}L (x_{\Scal_{k}},z )d_{\Scal_{k+1}}\right|}_{(\ref{ineq.expand.3.24.1}.1)}\\
&\hspace{2em} +\underbrace{\left|d_{\Scal_{k+1}}^T\nabla^2_{xx}L (x_{\Scal_{k}},z )d_{\Scal_{k+1}}-\dbar_{\Scal_{k+1}}^T\nabla^2_{xx}L (x_{\Scal_{k}},z )\dbar_{\Scal_{k+1}}\right|}_{(\ref{ineq.expand.3.24.1}.2)}\\
&\hspace{2em}+\underbrace{\left|\dbar_{\Scal_{k+1}}^T\nabla^2_{xx}L (x_{\Scal_{k}},z )\dbar_{\Scal_{k+1}}-\dbar_{\Scal_{k+1}}^T\nabla^2_{xx}L_{\Scal_{k}}(x_{\Scal_{k}},y_{\Scal_{k}})\dbar_{\Scal_{k+1}}\right|}_{(\ref{ineq.expand.3.24.1}.3)}.
\ealigned
\eequation
...
% $\(\eta_1\beta+\eta_3\)g_{\Scal_k}+\frac{3}{2} \eta_1\eta_2g_{\Scal_k}^2\le\frac{1}{4}\beta$, which gives 
...
% To ensure , we need , whose nonnegative solution is
