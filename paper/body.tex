...
\end{equation}
where the objective and constraint functions, i.e., $f : \R{n} \to \R{}$ and $\cbar : \R{n} \to \R{m}$, respectively, are twice-continuously differentiable, $m \leq n$, and the constraint function~$c$ is defined by an expectation.  Formally, with respect to a random variable $\omega$ defined by a probability space $(\Omega,\Fcal,\P)$, expectation $\E$ defined by $\P$, and $\Cbar : \R{n} \times \Omega \to \R{m}$, the constraint function $\cbar$ is defined by $\cbar(x) = \E[\Cbar(x,\omega)]$ for all $x \in \R{n}$.
...
\eequation
	...
	\bequationNN
	\|\nabla c_\Scal(x)-\nabla c (x)\|_2
	\le\sqrt{ \frac{2(\theta_J+\nu_J)N(N-|\Scal|)}{|\Scal|^2}}\|\nabla c (x)\|_2\le \frac{1}{3}\sigma_{\min},
\eequationNN
which further gives us
	\bequation
	\label{ineq.bound.psedueC.C}
		1-\|\nabla c (x)^\dag\|_2\|\nabla c (x)-\nabla c_\Scal(x)\|_2\ge1-\frac{1}{\sigma_{\min}}\frac{\sigma_{\min}}{3}=2/3.
	\eequation
	Hence, we have
	\bequation
	...
	(iii)
	&\le \frac{3}{2}\|\nabla c(x)^\dag\|_2\|\nabla c(x)-\nabla c_\Scal(x)\|_2\\
	&\le \frac{3\kappa_{\nabla c}}{ 2\sigma_{\min}}\sqrt{ \frac{2(\theta_J+\nu_J)N(N-|\Scal|)}{|\Scal|^2}}.
\ealigned
\eequation
...
\baligned
	...
	&=\|\Rcal\(\nabla c_\Scal(x)\)\Ncal\(\nabla c(x)\)\|_2\kappa_{\nabla f}\le \frac{\kappa_{\nabla c} \kappa_{\nabla f}}{\sigma_{\min}}\sqrt{2(\theta_J+\nu_J)} g_\Scal.
\ealigned
\eequation
...
\baligned
	...
	& \le \sqrt{\gamma_cN\(\frac{N-|\Scal|}{|\Scal|^2}\)} \|c(x)\|_2 \le \sqrt{\gamma_c} \kappa_cg_\Scal.
\ealigned
\eequation
...
\baligned
&\left|d_\Scal^T\nabla^2_{xx}L_\Scal(x,y_\Scal)d_\Scal\right|\\
&\ge \( \beta-\(\eta_1\beta+\eta_2 +3\eta_1\kappa_{\nabla^2 c}+\frac{9\eta_1\eta_2}{2\sqrt{\mu_H}}\)g_\Scal-\frac{3}{2} \eta_1\eta_2 g_\Scal^2\)\|d_\Scal\|_2^2\\
